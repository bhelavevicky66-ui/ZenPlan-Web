...
const WeeklyGoalSection: React.FC<WeeklyGoalSectionProps> = ({ goals, tasks, onAddGoal, onToggleGoal, onDeleteGoal }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white/50 backdrop-blur-md p-4 rounded-[2rem] border border-white shadow-sm">
        <div className="flex items-center gap-2">
           <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${weekInfo.isCurrentWeek ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-500'}`}>
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="w-10 h-10 flex items-center justify-center bg-white border border-slate-200 rounded-full hover:bg-slate-50 transition-all text-slate-600 active:scale-90"
          >
            <i className="fa-solid fa-chevron-left"></i>
          </button>
          
          <div className="bg-white px-8 py-2.5 rounded-2xl border border-slate-200 shadow-sm min-w-[180px] text-center">
            <span className="text-sm font-black text-indigo-600 tracking-tight">{weekInfo.rangeLabel}</span>
          </div>
        ...
        {!weekInfo.isCurrentWeek && (
          <button 
            ...
            className="text-xs font-bold text-indigo-600 hover:underline px-4"
          >
